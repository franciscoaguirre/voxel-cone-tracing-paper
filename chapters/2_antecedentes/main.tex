...
Tratar a la luz como una partícula es el campo de la óptima geométrica, en contraposición a la óptica física.
...
Los pasos programables son el \textit{vertex shader} (\textit{shader} de vértices), \textit{geometry shader} (geometría), y \textit{fragment} o \textit{pixel shader} (fragmentos o píxeles).
...
% TODO: Esto volverlo a poner, en teoría de pre-filtrado
% Tiene que ir esto y ray marching

% \section{Integral de rendering}

% Ya fue mencionada la ecuación de rendering \ref{eq:rendering_equation}.
