...
Cada téxel dentro de un brick se llama vóxel, porque además de ser un elemento de textura, también es un elemento de volumen, a los efectos de la aplicación.
En la figura \ref{fig:brick_pool_example} se muestra una \textit{brick pool}, con una textura 2D para facilitar la visualización, donde cada téxel es identificado por sus coordenadas $x$ e $y$ (y $z$ en su versión 3D).
...
Cada nodo de la \textit{node pool} está identificado por un numero, los cuáles pueden ser vistos en el margen izquierdo ($0, 1, 2$) de la figura \ref{fig:node_pool_example}.
...
Como se vió en \ref{sec:nodes_and_bricks}, cada nodo tiene un brick asociado.
