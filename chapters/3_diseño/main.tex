...
Es necesario calcular la intersección del plano del triangulo con los vóxles para encontrar los valores de profundidad en la grilla, ya que el triángulo fue proyectado perdiendo sus valores de profundidad.
...
La frontera entre dos nodos adyacentes del mismo nivel son los 9 vóxeles de la cara compartida entre los nodos, 3 en el caso 2D apreciable en las figuras, los cuales estan presentes en los bricks de ambos nodos. 
Como consecuencia la estructura tiene información repetida, lo cual es necesario para la interpolación a la hora de generar la imagen final funcione.
...
Se ejecuta el algoritmo especificado a continuación sobre cada nivel, para de esta forma generar el siguiente, hasta completarlo.
...
Dado un vóxel, se recorre el árbol construido hasta el momento, hasta llegar al último nivel actual.
Para reccorer el árbol, cada hilo comienza en el primer nodo del árbol.
Luego, en cada paso, se elige un hijo para descender utilizando la posición del vóxel en la grilla, iterativamente.
Al llegar a un nodo en el último nivel, dependendiendo de la posición del vóxel, se marca uno de los $8$ punteros a sus hijos.
Realizar esta marca es un proceso atómico e idempotente, ya que se limita a cambiar el bit mas significativo del puntero a 1.
...
A cada hilo se le asigna uno de los punteros de los nodos del nivel $i$, confirma si fue marcado en el paso anterior, elimina la marca, crea el nodo hijo y agrega al puntero el indice del hijo recien creado.
Siempre y cuando haya geometria en la región de la escena representada por un nodo, el mismo será subdividido nivel tras nivel hasta el maximo determinado por la resolución de la grilla de vóxeles.
...
En la figura \ref{fig:voxels-to-bricks} se puede observar un brick, cuyos vóxeles estan numerados, solapado con 4 vóxeles generados en el paso de voxelización, cada uno con su color.
Estos coinciden con el nodo del brick, ya que la grilla utilizada para la voxelización es la misma que la determinada por los nodos del nivel mas bajo, pero llegando a un nivel mayor de resolución.
En la figura, el vóxel 0 está una cuarta parte cubierto por el color violeta, por lo que se le asigna este valor. De forma similar se le asigna un color a las otras esquinas del brick. 

\begin{figure}[h!]
    \centering
    \includegraphics[width=.5\textwidth]{voxels-to-bricks.png}
    \caption{Solapamiento entre vóxeles de la voxelización y un brick}
...
La lógica detras del \textit{spread\_leaves} se desprende de la figura \ref{fig:voxels-to-bricks}, al ver las regiones de los vóxeles del brick que intersecan con los de la voxelización.
De esta figura también se desprende la necesidad del border transfer \ref{sec:border_transfer}, ya que los espacios blancos de los vóxeles del brick deben ser rellenados con información de nodos vecinos.
